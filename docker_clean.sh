...
#This is called as validation step in scripting
...
#Here first thing we are trying to do is check for all the images which dosen't have a proper id in it, 
#because all images that we create it will be cached and will be stored in /var/lib/docker/images
...
#Here we are checking if there are any containers which are stopped i.e Dead/Exited. We dont need them so we are cleaning them.
